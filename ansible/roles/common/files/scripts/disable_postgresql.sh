...
trigger_file="/etc/postgresql/11/main/im_the_master"
standby_file="/etc/postgresql/11/main/im_slave"
...
while test $# -gt 0; do
    ...
    case "$1" in
        ...
        -h|--help)
            ...
            echo "    Optional, default: /etc/postgresql/11/main/im_the_master"
            echo "-s, --standby_file=FILE   specify standby file path"
            echo "    Optional, default: /etc/postgresql/11/main/im_slave"
            echo " "
            echo "Error Codes:"
...
if [ -f /etc/postgresql/11/main/postgresql.conf ]; then
    ...
    if [ -f /etc/postgresql/11/main/postgresql.conf.disabled ]; then
        rm /etc/postgresql/11/main/postgresql.conf.disabled
    fi

    echo "INFO: Renaming postgresql.conf file to prevent future service start."
    mv /etc/postgresql/11/main/postgresql.conf /etc/postgresql/11/main/postgresql.conf.disabled

fi

# Deleting recovery.conf file
echo "INFO: Checking if recovery.conf file exists..."
if [ -f /var/lib/postgresql/11/main/recovery.conf ]; then

	echo "INFO: recovery.conf file found. Deleting..."
	
    rm /etc/postgresql/11/main/recovery.conf
fi
...
if [ -f /var/lib/postgresql/11/main/primary_info ]; then

	echo "INFO: primary_info file found. Deleting..."
	rm /var/lib/postgresql/11/main/primary_info
